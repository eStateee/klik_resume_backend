...
предметов, по которым у него есть уроки в целевом периоде.
...
--------------------
    Наставник (crm_id)
        ...
        -> Уроки этих групп за период ПН–ВС  (lesson/index, group_id + date_from/date_to)
...
Запуск
------
Вызывается из еженедельной Celery-таски (ночь с воскресенья на понедельник).
Период проверки — от ближайшего понедельника до конца ``LOOKAHEAD_WEEKS +
WINDOW_WEEKS`` недель (по умолчанию 4 недели). Начало от ближайшего ПН
гарантирует, что доступы к модулям на текущей/следующей неделе не теряются.
Логика выбора периода — см. :func:`get_week_range`.
...
#: Запас времени: сколько недель вперёд заглядываем за пределы ближайшей недели.
#: По умолчанию 2 — доступ к модулям выдаётся за две недели до начала занятий.
LOOKAHEAD_WEEKS: Final[int] = settings.CRM_LOOKAHEAD_WEEKS

#: Длина «дальнего» окна в неделях. По умолчанию 2 — захватываем две полные
#: недели ПН–ВС после сдвига LOOKAHEAD_WEEKS.
WINDOW_WEEKS: Final[int] = settings.CRM_WINDOW_WEEKS
...
def get_week_range(today: date | None = None) -> tuple[date, date]:
    """Вернуть границы периода ПН–ВС, за который нужно собрать предметы.

    Период покрывает от ближайшего понедельника до конца
    ``LOOKAHEAD_WEEKS + WINDOW_WEEKS`` недель. Начало от ближайшего ПН
    (без зазора) гарантирует, что доступы к модулям на текущей и следующей
    неделе не теряются.

    Синхронизация запускается ночью с воскресенья на понедельник, поэтому
    возможны два варианта момента запуска:

    * запуск в **воскресенье** (например, 23:50) — ближайший понедельник = завтра;
    * запуск в **понедельник** (например, 00:30) — ближайший понедельник = сегодня.

    Обе стороны полуночи дают один и тот же период. При ручном запуске
    в любой другой день недели за основу берётся понедельник текущей недели.

    Args:
        today: Дата, от которой считать период. По умолчанию — сегодня.

    Returns:
        Кортеж ``(monday, sunday)`` — включительные границы периода.

    Examples:
        >>> # LOOKAHEAD_WEEKS=2, WINDOW_WEEKS=2 (по умолчанию)
        >>> # Запуск в ВС 06.09 → период 07.09–04.10 (4 недели)
        >>> # Запуск в ПН 07.09 → тот же период 07.09–04.10
    """
    today = today or date.today()

    if today.weekday() == 6:  # 6 == воскресенье: ближайший понедельник — завтра
        base_monday = today + timedelta(days=1)
...
async def get_tutor_subject_ids(
    ...
    *,
    ...
    """Получить ``subject_ids`` предметов наставника за целевой период ПН–ВС.
